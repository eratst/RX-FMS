...
//						$scope.authModfiy = true;
		...
		//禁止导出按钮显示
		$scope.downloadBntShow = ($stateParams.id != 'T_IC_STDCMMM_CUAB') && ($stateParams.id != 'T_IC_STDDM_CUAB') && ($stateParams.id != 'T_IC_STDPRES_COEF') && ($stateParams.id != 'T_IC_VCF');
...
//						initGridOptionsUserU("T_PM_USER.json"); //仅在编辑弹框的初始化调用
		...
		function initGridOptionsColumnDefs(jsonObj) {
			...
			viewGridProvider.httpCommit(fileName).then(function success(res) {
				...
				$scope.gridOptions.columnDefs = viewGridProvider.formatGridOptionsColumnDefs(res.data.attribute, $scope.BntShow, $scope.updateShow, $scope.removeBnt, $scope.orgBtnShow, $scope.authModfiy, $scope.rentBtnShow);
			}, function error(res) {
				cj.Error(res);
			});
		};
		...
		function initGridOptionsData(curPage, pageSize) {
			if($scope.rentFlag) {
				var interUrl = viewGridProvider.httpPort() + jsonObj.url +
					'?$skip=' + (curPage - 1) * pageSize + '&$top=' + pageSize +
					viewGridProvider.getSearchKVUrl($scope.main.vMember.sapc.tableType);
			} else {
				var interUrl = viewGridProvider.httpPort() + jsonObj.url + "/" + localStorage.getItem('rentCode') +
					'?$skip=' + (curPage - 1) * pageSize + '&$top=' + pageSize +
					viewGridProvider.getSearchKVUrl($scope.main.vMember.sapc.tableType);
			...
			viewGridProvider.httpCommit(interUrl).then(function success(res) {
				...
				viewGridProvider.changeBooleanToCh($scope.main.vMember.sapc.tableType, $scope.gridOptions.data);
				$scope.gridApi.selection.clearSelectedRows();
		...
		$scope.main.vfunc.onclick.addButton = function() {
			...
			if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_PM_USER") {
				...
				for(var key in tableType.attribute) {
					if(key == 'userCode' || key == 'userName' || key == 'userOrgAlias' || key == 'jobDesc' || key == 'email' ||
						key == 'tel' || key == 'mobile' || key == 'sex' || key == 'birthday' || key == 'employeeId' || key == 'sortNum' || key == 'des' || key == 'inUse') {
		...
		$scope.main.vfunc.onclick.addDatasEnsure = function() {
			...
			for(var key in tableType.attribute) {
				if(key == 'capacity' || key == 'initialAssetValue' || key == 'netAssetValue') {
					if(tableType.attribute[key].proAdd.data) {
						tableType.attribute[key].proAdd.data = viewGridProvider.changeData(tableType.attribute[key].proAdd.data);
					}
					...
					if(tableType.attribute[key].proAdd.data) {
						tableType.attribute[key].proAdd.data = document.getElementById('getAddTime').value + " 00:00:00";
					}
			...
			if($scope.selectUserARelation.length > 0) {
				for(let i = 0; i < $scope.selectUserARelation.length; i++) {
					$scope.selectUserARelation[i].positionId = tableType.attribute['positionId'].proAdd.data;
					$scope.selectUserARelation[i].positionName = tableType.attribute['positionName'].proAdd.data;
					$scope.selectUserARelation[i].positionCode = tableType.attribute['positionCode'].proAdd.data;
			...
			console.log("增加所用url-->", viewGridProvider.httpPort() + jsonObj.url);
			viewGridProvider.httpPost(jsonObj.url, json).then(function success(res) {
				...
				if(res.data.collection.hasOwnProperty("templates")) {
					...
					initGridOptionsData($scope.gridOptions.paginationCurrentPage, $scope.gridOptions.paginationPageSize);
				} else {}
			}, function error(res) {
		...
		$scope.multiInsButtonOver = function() {
			...
			if(tableType.jsonObj.key == "T_PM_OUMORGUNIT") {
				...
				for(let i = 0; i < $scope.selectedList.length; i++) {
					...
					$scope.selectedList[i]['upperOrgCode'] = tableType.attribute['upperOrgCode'].proAdd.data;
					$scope.selectedList[i]['upperOrgAlias'] = tableType.attribute['upperOrgAlias'].proAdd.data;
		...
		$scope.main.vfunc.onclick.multiInsDatasEnsure = function() {
			...
			console.log("引入所用url-->", viewGridProvider.httpPort() + $scope.insertUrl);
			viewGridProvider.httpPost($scope.insertUrl, cj.parseCjArray($scope.selectedList)).then(function success(res) {
				console.info("引入返回数据res-->", res);
				if(res.data.collection.hasOwnProperty("templates")) {
					$scope.tempreset = "引入成功";
					$timeout(function() {
		...
		$scope.main.vfunc.onclick.multiInsTeamEnsure = function() {
			...
			viewGridProvider.httpPost($scope.insertTeamUrl, cj.parseCjArray($scope.selectedListTeam)).then(function success(res) {
		...
		function deleteData(i) {
			var upUrl = jsonObj.url + '/' + $scope.gridApi.selection.getSelectedRows()[i][$scope.main.vMember.sapc.tableType.alterKey];
		...
		$scope.main.vfunc.onclick.updateButton = function(row) {
			...
			if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_IC_CNFG_TANK") {
				var tankType = $scope.main.vMember.sapc.tableType.attribute['tankTypeCode'].proUpdate.data;
				...
				setcnfgClass(areaUrl);
				//班组用户关联获取用户
			} else if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_PM_TEAMANDUSER") {
				var orgAlias = $scope.main.vMember.sapc.tableType.attribute['userOrgAlias'].proUpdate.data;
			...
			viewGridProvider.setUpdateData($scope.gridApi.selection.getSelectedRows()[0], $scope.main.vMember.sapc.tableType);
		}
		/**
		 * “修改”确认事件
		 */
		$scope.main.vfunc.onclick.updataEnsure = function() {
			...
			for(var key in tableType.attribute) {
				if(key == 'capacity' || key == 'initialAssetValue' || key == 'netAssetValue') {
					if(tableType.attribute[key].proUpdate.data) {
						tableType.attribute[key].proUpdate.data = viewGridProvider.changeData(tableType.attribute[key].proUpdate.data);
					}
				} else if(key == 'parentOrgAlias') {
					if(tableType.attribute[key].proUpdate.data == '') {
			...
			var alterUrl = jsonObj.url + '/' + $scope.alterValue;
		...
		$scope.main.vfunc.onclick.serchButton = function() {
			viewGridProvider.setSearchFlag($scope.main.vMember.sapc.tableType, $scope.main.vMember.sapc.tableType.jsonObj)
		}
		$scope.main.vfunc.onclick.serchOver = function() {
			...
			for(let item in tableType.attribute) {
				...
				} else if(item == 'orgAlias' && (tableType.jsonObj.nameAlias == "装置" || tableType.jsonObj.nameAlias == "罐区" || tableType.jsonObj.nameAlias == "仓库" ||
						tableType.jsonObj.nameAlias == "装卸台" || tableType.jsonObj.nameAlias == "管网(非能源管网)" || tableType.jsonObj.nameAlias == "生活区" ||
						tableType.jsonObj.nameAlias == "办公区" || tableType.jsonObj.nameAlias == "侧线" || tableType.jsonObj.nameAlias == "罐" || tableType.jsonObj.nameAlias == "设备" ||
						tableType.jsonObj.nameAlias == "进出厂点" || tableType.jsonObj.nameAlias == "料仓" || tableType.jsonObj.nameAlias == "库位" || tableType.jsonObj.nameAlias == "采样点" ||
						tableType.jsonObj.nameAlias == "排放口" || tableType.jsonObj.nameAlias == "管段" || tableType.jsonObj.nameAlias == "阀门" || tableType.jsonObj.nameAlias == "盲板" ||
						tableType.jsonObj.nameAlias == "三通" || tableType.jsonObj.nameAlias == "装卸点" || tableType.jsonObj.nameAlias == "罐量计算基础配置" || tableType.jsonObj.nameAlias == "罐量计算单罐配置")) {
					if(!tableType.attribute[item].proSearch.data) {
		...
		$scope.main.vfunc.onclick.serchEnsure = function() {
			...
			str = viewGridProvider.httpPort() + jsonObj.url +
				'?$skip=' + ($scope.gridOptions.paginationCurrentPage - 1) * $scope.gridOptions.paginationPageSize +
		...
		$scope.main.vfunc.onclick.importEnsure = function() {
			...
			for(var i = 0; i < $scope.mTable.tableValue.length; i++) {
				...
				for(var key in tableValue) {
					...
					if(key == 'inUse' || key == 'techcardCtlFlag' || key == 'isPublic' || key == 'isInnerOp' ||
						key == 'monFlag' || key == 'ptrlFlag' || key == 'htPretTank' || key == 'dataStatus' ||
						key == 'tankIdt' || key == 'isUseFormula') {
				...
				var repeatUrl;
				if(jsonObj.key == 'T_PM_ASSOCIATIVE') {
					repeatUrl = viewGridProvider.httpPort() + jsonObj.url + '/' + $scope.mTable.tableValue[i][nodeA] + '/' + $scope.mTable.tableValue[i][nodeB];
				}else if(jsonObj.key == 'T_PM_BIZORG_DTL') {
					repeatUrl = viewGridProvider.httpPort() + jsonObj.url + '/' + $scope.mTable.tableValue[i][nodeC] + '/' + $scope.mTable.tableValue[i][nodeD];
				} else if(jsonObj.key == 'T_IC_SIMPLE_NODE_MAP') {
					repeatUrl = viewGridProvider.httpPort() + jsonObj.url + '/' + $scope.mTable.tableValue[i][nodeS] + '/' + $scope.mTable.tableValue[i][nodeM];
				} else {
					repeatUrl = viewGridProvider.httpPort() + jsonObj.url + '/' + $scope.mTable.tableValue[i][alterKey];
				}
				console.log('repeatUrl', repeatUrl)
				searchImport(repeatUrl, importJson, i, alterKey, $scope.mTable.tableValue[i][alterKey], nodeA, $scope.mTable.tableValue[i][nodeA], nodeB, $scope.mTable.tableValue[i][nodeB], nodeC, $scope.mTable.tableValue[i][nodeC], nodeD, $scope.mTable.tableValue[i][nodeD], nodeM, $scope.mTable.tableValue[i][nodeM], nodeS, $scope.mTable.tableValue[i][nodeS])
			};
		}

		function searchImport(repeatUrl, importJson, index, alterKey, alterValue, nodeA, nodeAValve, nodeB, nodeBValve,nodeC, nodeCValve, nodeD, nodeDValve, nodeM, nodeMValve, nodeS, nodeSValve) {
			viewGridProvider.httpCommit(repeatUrl).then(function success(res) {
				console.log('ressss', res)
				if((alterValue || nodeAValve|| nodeCValve || nodeSValve) && res.data.collection.items.length == 1) {
					var putUrl;
					if(jsonObj.key == 'T_PM_ASSOCIATIVE ') {
		...
		function importPostData(i, importJson, postUrl) {
			...
			viewGridProvider.httpPost(postUrl, JSON.stringify(importJson)).then(function success(res) {
				if(res.data.collection.hasOwnProperty("templates")) {
					...
					if($scope.imSuccess.length + $scope.imError.length == $scope.mTable.tableValue.length) {
		...
		$scope.buttonOver = function(whichflag) {
			...
			if(tableType.jsonObj.key == 'T_PM_BIZORG_DTL') {
				if(tableType.attribute['orgCode'][whichflag].data == tableType.attribute['parentOrgCode'][whichflag].data) {
					if(tableType.attribute['orgCode'][whichflag].data != null && tableType.attribute['parentOrgCode'][whichflag].data != '') {
				...
				if(key == 'cnfgClassId') {
					...
					for(var i = 0; i < $scope.cnfgClass.length; i++) {
						if($scope.cnfgClass[i].cnfgClassName == cnfgClassNames) {
							$scope.main.vMember.sapc.tableType.attribute['cnfgClassCode'][proS].data = $scope.cnfgClass[i].cnfgClassCode;
							$scope.main.vMember.sapc.tableType.attribute['cnfgClassId'][proS].data = $scope.cnfgClass[i].cnfgClassId;
						}
					}
				}
			}
...
//					}
		...
		$scope.main.vfunc.onclick.treeButtonStd = function(valueProStd, proStdFlag, treeFlag) {
			...
			viewGridProvider.httpCommit(bizUrl).then(function success(res) {
				...
				for(let i = 0; i < $scope.stdTreeRes.length; i++) {
					if($scope.stdTreeRes[i].upperOrgId == '' || $scope.stdTreeRes[i].upperOrgId == 0) {
						let obj = {}
						obj.ifExpand = true;
			...
			for(let item in $scope.main.vMember.sapc.tableType.attribute) {
				if((item == 'upperOrgCode' || item == 'parentOrgCode') && $scope.treeFlag == 'parTree') {
					...
					if($scope.proStdFlag == 'proSearch') {
						...
						$scope.main.vMember.sapc.tableType.attribute[item][$scope.proStdFlag].data = orgStd.orgCode;
					}
					...
					if($scope.proStdFlag == 'proSearch') {
						...
						if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_PM_BIZORG_DTL") {
							...
							$scope.main.vMember.sapc.tableType.attribute['orgTypeName'].proSearch.data = orgStd.orgTypeName;
						}
					} else {
						$scope.main.vMember.sapc.tableType.attribute[item][$scope.proStdFlag].data = orgStd.orgCode;
						//console.log('--------', $scope.main.vMember.sapc.tableType.attribute)
						if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_PM_BIZORG_DTL") {
							$scope.main.vMember.sapc.tableType.attribute['orgTypeName'][$scope.proStdFlag].data = orgStd.orgTypeName;
							$scope.main.vMember.sapc.tableType.attribute['parentOrgAlias'][$scope.proStdFlag].data = orgStd.upperOrgAlias;
							$scope.main.vMember.sapc.tableType.attribute['parentOrgCode'][$scope.proStdFlag].data = orgStd.upperOrgCode;
						}
					}
		...
		$scope.bizOrgTree = function(biz) {
			...
			if(biz.bizCode == localStorage.getItem('rentCode') + '_SYSTEM_STANDARD_BIZ') {
				...
				topUrl = viewGridProvider.httpPort() + localStorage.getItem('bizOrgUrl') + $scope.bizCode + '/tree?$opertype=root';
			...
			viewGridProvider.httpCommit(_url).then(function success(res) {
				...
				if(resultArr.length > 0) { //如果resultArr.length === 0, 则没有下一级。
					...
					for(var i = 0; i < resultArr.length; i++) {
						var childUrl = viewGridProvider.httpPort() + localStorage.getItem('bizOrgUrl') + $scope.bizCode + '/tree/' +
		...
		$scope.main.vfunc.onclick.orgOnClick = function(forthElem) {
			...
			for(let item in $scope.main.vMember.sapc.tableType.attribute) {
				if(item == 'orgCode' && $scope.main.vMember.sapc.tableType.jsonObj.nameAlias != "用户" && $scope.main.vMember.sapc.tableType.jsonObj.nameAlias != "班组用户关联") {
					if($scope.proFlag == 'proSearch') {
						$scope.main.vMember.sapc.tableType.attribute[item].proSearch = {};
						$scope.main.vMember.sapc.tableType.attribute[item].proSearch.data = forthElem.orgCode;
					} else {
						$scope.main.vMember.sapc.tableType.attribute[item][$scope.proFlag].data = forthElem.orgCode;
					}
				} else if(item == 'userOrgCodes' && ($scope.main.vMember.sapc.tableType.jsonObj.nameAlias == "用户" || $scope.main.vMember.sapc.tableType.jsonObj.nameAlias == "班组用户关联")) {
					if($scope.proFlag == 'proSearch') {
						$scope.main.vMember.sapc.tableType.attribute[item].proSearch = {};
						$scope.main.vMember.sapc.tableType.attribute[item].proSearch.data = forthElem.orgCode;
					} else {
		...
		$scope.main.vfunc.onclick.areaOnClick = function(proWhich) {
			...
			} else if(tableType.jsonObj.key == "T_PM_TANK" || tableType.jsonObj.key == "T_IC_CNFG_BASE" || tableType.jsonObj.key == "T_IC_CNFG_TANK") {
			...
			if(proWhich == 'proSearch') {
				if($scope.levelOrg) {
					areaUrl = viewGridProvider.httpPort() + url + "?inUse=1&dataStatus=1&isRecursive=1&orgCode=" + urlCode + '&$nodeType=' + nodeType;
				} else if(!$scope.levelOrg) {
					areaUrl = viewGridProvider.httpPort() + url + "?inUse=1&dataStatus=1&isRecursive=0&orgCode=" + urlCode + '&$nodeType=' + nodeType;
				}
			} else {
				areaUrl = viewGridProvider.httpPort() + url + "?inUse=1&dataStatus=1&isRecursive=1&orgCode=" + urlCode + '&$nodeType=' + nodeType;
			}
		...
		$scope.main.vfunc.onclick.classOnClick = function(forthElem) {
			if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_IC_CNFG_TANK") {
				...
				var url = viewGridProvider.getTableIndexJsonByKey(tableIndex, "T_IC_CNFG_CLASS").url;
				var areaUrl = localStorage.getItem('serverUrl') + url + '?inUse=1&dataStatus=1' + '&tankType=' + tankType;
				setcnfgClass(areaUrl);
			} else if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_IC_CNFG_BASE") {
				var url = viewGridProvider.getTableIndexJsonByKey(tableIndex, "T_IC_CNFG_CLASS").url;
		...
		$scope.main.vfunc.onclick.userOnClick = function(forthElem) {
			...
			if($scope.main.vMember.sapc.tableType.jsonObj.key == "T_PM_TEAMANDUSER") {
				var userOrgCodes = $scope.main.vMember.sapc.tableType.attribute['userOrgCodes'][forthElem].data;
				userUrl = localStorage.getItem('serverUrl') + url + '?inUse=1&dataStatus=1' + '&userOrgCodes=' + userOrgCodes;
		...
		$scope.main.vfunc.onclick.selectUserName = function(pro, userName) {
			for(let i = 0; i < $scope.userNameList.length; i++) {
				if(userName == $scope.userNameList[i].userName) {
					$scope.main.vMember.sapc.tableType.attribute['userId'][pro].data = $scope.userNameList[i].userId;
					$scope.main.vMember.sapc.tableType.attribute['userCode'][pro].data = $scope.userNameList[i].userCode
					$scope.main.vMember.sapc.tableType.attribute['userName'][pro].data = $scope.userNameList[i].userName
				}
			}
		}
				...
				if(key == 'cnfgClassId') {
					for(var i = 0; i < $scope.cnfgClass.length; i++) {
						if($scope.cnfgClass[i].cnfgClassName == cnfgClassName) {
							$scope.main.vMember.sapc.tableType.attribute['cnfgClassCode'].proUpdate.data = $scope.cnfgClass[i].cnfgClassCode;
							$scope.main.vMember.sapc.tableType.attribute['cnfgClassId'].proUpdate.data = $scope.cnfgClass[i].cnfgClassId;
						}
					}
				}
			}
		...
		function initGridOptionsUserA(jsonObj) {
			...
			viewGridProvider.httpCommit(fileName).then(function success(res) {
				viewGridProvider.hideCrtMnt(res.data);
				$scope.userOptionsAdd.columnDefs = viewGridProvider.formatGridOptionsColumnDefs(res.data.attribute, false, $scope.updateShow, $scope.removeBnt, $scope.orgBtnShow);
			}, function error(res) {
				cj.Error(res);
			});
		};
		...
		function initAGridOptionsUserRelation(curPage, pageSize, searchStr) {
			if(searchStr) {
				var userRelationUrl = viewGridProvider.httpPort() +
					'/users?$skip=' + (curPage - 1) * pageSize + '&$top=' + pageSize + searchStr + '&inUse=1&dataStatus=1';
			} else {
				var userRelationUrl = viewGridProvider.httpPort() +
			...
			viewGridProvider.httpCommit(userRelationUrl).then(function success(res) {
				if(res.data.collection.hasOwnProperty('items')) {
					if(res.data.collection.items.length > 0) {
						for(var i = 0; i < res.data.collection.page.data.length; i++) {
							if(res.data.collection.page.data[i].name == "recordCount" || res.data.collection.page.data[i].name == "totalElements") {
								$scope.userOptionsAdd.totalItems = res.data.collection.page.data[i].value;
							}
						}
						$scope.userOptionsAdd.data = cj.Parse(res.data, true);
						viewGridProvider.changeBooleanToCh($scope.main.vMember.sapc.tableType.attribute.userName.parentType, $scope.userOptionsAdd.data);
					} else {
						angular.element("#tempresetModel").modal("show");
						$scope.tempreset = "查询失败";
						$scope.error = '没有该记录';
		...
		$scope.main.vfunc.onclick.userAddEnsure = function() {
			...
			if($scope.selectUserARelation.length > 0) {
				for(var key in tableType.attribute) {
					if(key == "userCode") {
						tableType.attribute['userCode'].proAdd.data = $scope.selectUserARelation[0]['userCode'];
					} else if(key == "userId") {
						tableType.attribute['userId'].proAdd.data = $scope.selectUserARelation[0]['userId'];
					} else if(key == "userName") {
						tableType.attribute['userName'].proAdd.data = $scope.selectUserARelation[0]['userName'];
					}
				}
			}
		}
		...
		$scope.main.vfunc.onclick.serchAUserEnsure = function() {
			var str = viewGridProvider.getSearchKVUrl($scope.main.vMember.sapc.tableType.attribute.userName.parentType);
		...
		function initGridOptionsUserU(jsonObj) {
			...
			viewGridProvider.httpCommit(fileName).then(function success(res) {
				viewGridProvider.hideCrtMnt(res.data);
				$scope.userOptionsUpdate.columnDefs = viewGridProvider.formatGridOptionsColumnDefs(res.data.attribute, false, $scope.updateShow, $scope.removeBnt, $scope.orgBtnShow);
			}, function error(res) {
				cj.Error(res);
			});
		};
			...
			viewGridProvider.httpCommit(userRelationUrl).then(function success(res) {
				...
				if(res.data.collection.hasOwnProperty('items')) {
					if(res.data.collection.items.length > 0) {
						for(var i = 0; i < res.data.collection.page.data.length; i++) {
							if(res.data.collection.page.data[i].name == "recordCount" || res.data.collection.page.data[i].name == "totalElements") {
								$scope.userOptionsUpdate.totalItems = res.data.collection.page.data[i].value;
							}
						}
						$scope.userOptionsUpdate.data = cj.Parse(res.data, true);
						viewGridProvider.changeBooleanToCh($scope.main.vMember.sapc.tableType.attribute.userName.parentType, $scope.userOptionsUpdate.data);
					} else {
						angular.element("#tempresetModel").modal("show");
						$scope.tempreset = "查询失败";
						$scope.error = '没有该记录';
		...
		$scope.main.vfunc.onclick.userUpdateEnsure = function() {
			...
			if($scope.selectUserURelation.length > 0) {
				for(var key in tableType.attribute) {
					if(key == "userCode") {
						tableType.attribute['userCode'].proUpdate.data = $scope.selectUserURelation[0]['userCode'];
					} else if(key == "userId") {
						tableType.attribute['userId'].proUpdate.data = $scope.selectUserURelation[0]['userId'];
					} else if(key == "userName") {
						tableType.attribute['userName'].proUpdate.data = $scope.selectUserURelation[0]['userName'];
					}
				}
			}
		}
