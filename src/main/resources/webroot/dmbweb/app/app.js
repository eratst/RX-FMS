...
//					console.log("\\\\\\\\\\\\", tableType)
				...
				getHttpData: function(flag, tableType) {
					...
					if(flag == 'proAdd') {
						for(var key in tableType.attribute) {
							//如果有值
							if(tableType.attribute[key].proAdd.data || tableType.attribute[key].proAdd.data === 0|| tableType.attribute[key].proAdd.data ===false) {
					...
					if(flag == 'proUpdate') {
						for(var key in tableType.attribute) {
							...
							if(tableType.attribute[key].proUpdate.data || tableType.attribute[key].proUpdate.data === 0) {
								...
								} else if(tableType.attribute[key].proUpdate.data == "消耗"&&tableType.attribute[key].valueShowLine) {
									tableType.attribute[key].proUpdate.data = 2;
								...
								}else if(tableType.attribute[key].proUpdate.data == "消耗"&&tableType.attribute[key].valueShowTy){
									tableType.attribute[key].proUpdate.data = 0;
								}else if(tableType.attribute[key].proUpdate.data == "自产"){
									tableType.attribute[key].proUpdate.data =1;
								}
...
//							}
				 ...
				 * 初始化设置属性是否显示和可编辑
				 */
				setPartFlag: function(tableType,measIndex) {
					var obj = {};
					for(var key in tableType.attribute) {
						/**增加及修改的pro初始化开始*/
...
//										}
				...
				changeBooleanToCh: function(tableType, data) {
					for(var key in tableType.attribute) {
						...
						if(tableType.attribute[key].valueShowTy) {
							for(var i = 0; i < data.length; i++) {
								data[i][key] = data[i][key] ? '自产' : '消耗';
							}
						}
